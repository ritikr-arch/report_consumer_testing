                                        ...
                                        <div class="card-body">
                                            <div class="row">
                                                <div class="col-12 col-md-6 col-lg-4 mb-3">
                                                    ...
                                                    <label for=""> Type <span class="text-danger">*</span>&nbsp;<span id="type_error"></span></label>
                                            
                                                    <select name="type[]" id="type"  class="form-control" style="width:100% !important;">
                                                ...
                                                ?>
                                               <div class="col-md-4 col-12 mb-3">
    <label for="start_date">Date <span class="text-danger">*</span>&nbsp;<span id="start_date_error"></span></label>
                    ...
                    @if (isset($data) && count($data) > 0)
                        <div class="col-xl-12 mx-auto mt-3 ">
                            <div class="table-height-store2 mb-5">
                                <div class="zone-child my-4 d-flex justify-content-between
" style="width:100%;">
                                ...
                                <div class="d-flex">
                                    ...
                                    <div>
                                        <a class="filterBtn" style="color:white;" href="{{route('frontend.export.products.report', request()->query())}}">Export</a>
                                    </div> 
...
<button id="btnList" class="toggle-btn btn custom-outline-btn btn-sm">
    ...
    $accord_id = 1; // globally unique ID counter
    ?>
    ...
    <div class="accordion" id="mainAccordion">
        @foreach ($datalist as $surveyNameList => $datalist)
            @php
                ...
                $marketIds = SubmittedSurvey::where('survey_id', $surverID)->pluck('market_id')->unique()->toArray();
            ...
            <div class="bg-dark text-white px-3 py-2 text-center">
                Price Collected On: {{ customt_date_format($surveyDatas->start_date) }} ({{ $zone_name->name }})
            </div>
            ...
            @foreach ($datalist['categories'] as $categoryName => $categoryData)
                ...
                <div class="accordion-item" id="category_{{ $accord_id }}">
                    <h2 class="accordion-header" id="heading_{{ $accord_id }}">
                        <button class="accordion-button collapsed bg-light rounded-0 border-top-0 py-2"
                                type="button"
                                    ...
                                    @foreach ($categoryData->groupBy('commodity_id') as $commodityId => $commodityData)
                                        ...
                                        <div class="accordion-item">
                                            <h2 class="accordion-header"
                                                ...
                                                <button class="store-price accordion-button commodity-accordion-item collapsed px-3 py-2 d-flex justify-content-between align-items-center"
                                                        type="button"
                                                        data-bs-toggle="collapse"
                                                       ...
                                                       <span>
                                                            ...
                                                            <i class="fas fa-minus"></i>🧃 {{ !empty($commodityName) ? $commodityName : 'N/A' }}

🏷️ {{ !empty($firstRow->brand->name) && strtolower($firstRow->brand->name) !== 'no name' ? $firstRow->brand->name : 'N/A' }}

⚖️ {{ !empty($firstRow->unit->name) ? $firstRow->unit->name : 'N/A' }}
                                                ...
                                                <div class="accordion-body ps-3">
                                                    ...
                                                    @foreach ($markets as $market)
                                                        @php
                                                            ...
                                                            $priceColor = '';
                                                            if ($marketPrice && !$isSamePrice) {
                                                                if ($marketPrice->amount == $minPrice) {
                                                                    $priceColor = '#228522 !important';
                                                                } elseif ($marketPrice->amount == $maxPrice) {
                                                                    $priceColor = '#e52929 !important';
                                                                }
                                                            }
                                                        @endphp

                                                        <div class="store-price px-3">
                                                              <span>{{ ucfirst($market->name) }}</span>
                                            ...
                                            @if (isset($data) && count($data) > 0)
                                                @foreach ($data as $surveyName => $data)
                                                    ...
                                                    <thead>
                                                        ...
                                                        <tr bgcolor="#f1f1f1" class="zone-tbl-head">
                                                            ...
                                                            <div>@if (isset($markets) && count($markets) > 0)
                                                                @foreach ($markets as $market)
                                                                    <th align="center" class="text-center" colspan="{{ 7 - count($markets) }}">{{ ucfirst($market->name) }}</th>
                                                                @endforeach
                                                            @endif</div>
                                                    ...
                                                    @foreach ($data['categories'] as $categoryName => $categoryData)
                                                        <!-- Category Row -->
                                                        <tr>
                                                            <td colspan="{{ 3 + (count($markets) < 7 ? 7 : count($markets)) }}"
                                                        ...
                                                        @foreach ($categoryData->groupBy('commodity_id') as $commodityId => $commodityData)
                                                            ...
                                                            <tr>
                                                                ...
                                                                @foreach ($markets as $market)
                                                                    ...
                                                                    @endphp
                                                                    <td
                                                                        style="background-color: {{ $bgColor }}; color: {{ $textColor }}; font-weight: {{ $fontWeight }}" class="text-center" colspan="{{ 7 - count($markets) }}">
                                                                        {{ $marketPrice && $marketPrice->amount ? '$' . $marketPrice->amount : '-' }}
                                                                    </td>
                                                                @endforeach
                                                            </tr>
        ...
        $(document).ready(function () {
            ...
            $('#searchReport').on('submit', function (e) {
                ...
                if ($('#market').val() === null || $('#market').val().length === 0) {
                    isValid = false;
                    showError('#market_error', '( Please select at least one Store )');
                }
