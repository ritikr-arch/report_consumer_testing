...
<div class="px-3">
  ...
  <div class="container-fluid">
    <form action="{{route('admin.about.update')}}" method="post" id="about_us_form" enctype="multipart/form-data">
      @csrf
      <div class="row mt-3">
        <div class="col-xl-12">
          <div class="card">
            <div class="card-body">
              <div class="row align-items-center d-flex mb-3">
                <div class="col-xl-12">
                  <h4 class="header-title mb-0 font-weight-bold"> About Us </h4>
                </div>
              </div>
              <div class="row">
                <div class="col-md-6">
                  <label for="companyLogoUpload">Title <span class="text-danger">*</span>
                  </label>
                  <input type="text" class="form-control" id="exampleFormControlInput1" placeholder="Enter Title" maxlength="250" name="title" value="{{old('title', @$data->title)}}"> @error('title') <small class="text-danger text-bold"> {{ $message }} </small> @enderror
                </div>
                <div class="col-md-5">
                  <label for="companyLogoUpload">Image Upload <span class="text-danger">*</span>
                  </label>
                  <input type="file" class="form-control" name="image" accept="image/*" style="margin-top:6px;">
                  <p style="font-size: 11px; font-weight: 700;"> Recommended Dimension -> 543x412 Pixels (Max) </br> Image size should not more then 5MB </p>
                ...
                <div class="col-md-1">
                  <div class="profile-background">
                    @if(!empty($data->image))
                    <img src="{{ asset('admin/images/cms/'.$data->image) }}" id="companyLogoPreview" class="profile-image" width="100" height="100">
                    @endif
                  </div>
                </div>
                <div class="col-md-12">
                  <div class="form-group ad-user mt-4">
                    <label for="exampleFormControlInput1">Content <span class="text-danger">*</span>
                    </label>
                    <textarea class="form-control oye-f2" name="content" id="content">{{ old ('content', @$data->content)}}</textarea>
                  </div> @error('content') <small class="text-danger text-bold"> {{ $message }} </small> @enderror
                </div>
            ...
            editor.on('init', function () {
                $("#about_us_form").validate({
                    ...
                    submitHandler: function (form) {
                        ...
                        const wrapper = $('<div>').html(contentField.val());
                        ...
                        wrapper.find('p').each(function () {
                            ...
                            if ($images.length) {
                                $images.each(function () {
                                    ...
                                    // Insert image in its own <p> above current <p>
                                    $p.before($('<p>').append($img));
                                    // Remove the original image from the current <p>
                                    $(this).remove();
                                });
                            }
                        });

                        contentField.val(wrapper.html()); // update textarea value
