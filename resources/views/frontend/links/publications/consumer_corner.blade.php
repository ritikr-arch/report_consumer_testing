...
<div class="breadcumb-wrapper background-image" style="background-image: url(' {{ asset('frontend/img/bread-crum.jpg') }} ') ">
  <div class="container">
    <div class="breadcumb-content">
      <h2 class="breadcumb-title1">Consumer Corner</h2>
    </div>
  </div>
</div>
...
<div class="container">
   <div class="row">
      <div class="card-listing col-12 col-md-8 ps-4">
         <ul>
            @if(!empty($consumer_corner) && count($consumer_corner)>0)

            @foreach($consumer_corner as $values)
            <li>
    ...
    $url = ($values['type'] === 'video_external' || $values['type'] === 'link') 
        ? $file 
        : asset('admin/images/consumer_corner/' . $file);
@endphp
